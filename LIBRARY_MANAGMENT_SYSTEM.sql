...
select custid,concat(fname,' ',lname) as fullname from customer
where custid not in (select distinct custid from transaction);

/*2. Select book with highest page count*/
select bkid,title from book
 where pagecount in (select max(pagecount) from book);
 

/*3. Select customer who has borrowed some book for longest period*/
select customer.custid,customer.fname,datediff(return_date,issue_date) as period from customer
join transaction on customer.custid=transaction.custid order by period desc limit 1;

/*JOINS:*/

/*1. Display list of all books along with their authors*/
select book.bkid,book.title,concat(fname,' ',lname) as authorname from book
left join author on book.authorid=author.authorid; 

/*2. Display list of top 20 most borrowed books*/
select book.bkid, book.title, count(transaction.bkid) as books_borrowed
from book
join transaction on book.bkid=transaction.bkid
group by bkid,title order by books_borrowed desc limit 20;

/*3. Display list of top 10 customers who have borrowed most books*/
SELECT Customer .custID, CONCAT(Customer .Fname, ' ', Customer .Lname) AS Name, 
COUNT(Transaction .transID) AS Books_Borrowed
FROM Customer 
